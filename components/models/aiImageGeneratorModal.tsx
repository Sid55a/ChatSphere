...
export const AiImageGeneratorModel = () => {
  const router = useRouter();
  const [isMounted, setIsMounted] = useState(false);
  const { isOpen, onClose, onOpen, type } = useModal();
  const isModelOpen = isOpen && type === "imageGeneration";
  ...
  async function query(data: any) {
    const response = await fetch(
      "https://api-inference.huggingface.co/models/Yntec/HELLmix",
      {
        headers: {
          Authorization: "Bearer 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",
        },
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      query({ inputs: values.name }).then((response) => {
        console.log(response);
      });
      // setAiImageResult(response?.data?.result);
      form.reset();
    } catch (error) {
      console.log(error);
    }
